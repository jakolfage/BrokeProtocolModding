...
namespace AssetBundleBrowser
{
    ...
    internal class BundleDetailList : TreeView
    {
        ...
        internal static TreeViewItem AppendBundleToTree(BundleDataInfo bundle)
        {
            ...
            var depList = bundle.BundleDependencies;
            if(depList.Count > 0)
            {
                dependency.displayName = k_DependencyHeader;
                foreach (var dep in bundle.BundleDependencies)
                {
                    ...
                    for( int i = 0; i < dep.m_FromAssets.Count; ++i )
                    {
                        ...
                        TreeViewItem refItem = new TogglePathTreeViewItem( str.GetHashCode(), 4, k_ReferencedPrefix, dep.m_FromAssets[i].DisplayName, dep.m_FromAssets[i].FullAssetName );
            ...
            if (bundle.HasMessages)
            {
                ...
                var currMessages = bundle.Messages;
